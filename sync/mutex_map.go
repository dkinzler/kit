// Package sync provides advanced synchronization components.
package sync
...
// I.e. only one goroutine at a time can hold the lock for the same key, locks for different keys are independent.
// A use case might for example be a HTTP service that processes requests concurrently, but we want to make sure that at most one request for the same user is handled at a time.
// To achieve this, one can share a single MutexMap across all http handler goroutines and the http handler obtains the lock for the user (by e.g. using a userId as the key for the MutexMap) before doing any work.
//
// Example usage:
//
//	mm := NewMutexMap()
...
// The calling goroutine can use the returned Unlocker to unlock the lock for the given key when done.
